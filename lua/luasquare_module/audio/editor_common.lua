...
function EDITOR.OpenSharedPool(editor)
    ...
    EDITOR.RegisterSubwindow(frame)
    local status = LUASQUARE_EDITOR_THEME.CreateTextArea(frame) status:Dock(BOTTOM) status:SetTall(42)
...
function EDITOR.BuildFrame(title, source, category, mapOwned, bodyBuilder)
    ...
    frame.Status = LUASQUARE_EDITOR_THEME.CreateTextArea(frame) frame.Status:Dock(BOTTOM) frame.Status:SetTall(34)
